...
export default function ExtractPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    form.append("file", file); form.append("pages", JSON.stringify(pages));
    try {
      setProgress(50);
      const res = await fetch("/api/extract", { method: "POST", body: form });
      setProgress(90);
      if (!res.ok) { const d = await res.json() as { error: string }; throw new Error(d.error); }
      triggerDownload(await res.blob(), "extracted.pdf");
      ...
      <div className="max-w-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="rounded-2xl bg-white p-6 shadow-sm space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Páginas a extraer</label>
              <input type="text" value={pagesInput} onChange={(e) => setPagesInput(e.target.value)} placeholder="Ej: 1, 3, 5-8, 11-13" className={inputCls} />
              <p className="mt-1 text-xs text-gray-400">Ejemplos: 1, 3, 5-8</p>
            </div>
          </div>
          {file && pagesInput && <StatusMessage status="info" message="Se generará un nuevo PDF solo con las páginas seleccionadas." />}
