...
import { verifyMetadata } from "@/lib/pdf/verify-metadata";
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const result = await verifyMetadata(buffer, file.size);

    console.info({ op: "verify-metadata", pageCount: result.pageCount });

    return Response.json(result, { status: 200 });
  } catch (err) {
    const message = err instanceof Error ? err.message : "";
    console.error({ op: "verify-metadata", error: message });
